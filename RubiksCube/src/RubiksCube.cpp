...
RubiksCube::RubiksCube(): rotationAngle(glm::radians(-90.0f)), cubies{}
{
    for(int x = -1; x <= 1; x++) {
        for(int y = -1; y <= 1; y++) {
            for(int z = -1; z <= 1; z++) {
                int index = (x + 1) * 9 + (y + 1) * 3 + (z + 1);
                cubies[index].position = OFFSET * glm::vec3(x, y, z);
                cubies[index].rotationMatrix = glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f));
            }
        }
    }
}
...
void RubiksCube::rotate(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, glm::vec3 axis)
{
    ...
    std::array<Cubie, 27> newCubies = cubies;
    ...
    for(int x = minX; x <= maxX; x++) {
        for(int y = minY; y <= maxY; y++) {
            for(int z = minZ; z <= maxZ; z++) {
                ...
                cubies[index].rotationMatrix = rotation * cubies[index].rotationMatrix;
                cubies[index].position = glm::vec3(transformation * glm::vec4(cubies[index].position, 1.0f));
                ...
                newCubies[newIndex] = cubies[index];
            }
        }
    }
